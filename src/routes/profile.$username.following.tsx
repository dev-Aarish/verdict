...
import { getFollowingListFn, followUserFn, unfollowUserFn, getFollowStatusFn } from "@/api/follows";
...
export const Route = createFileRoute("/profile/$username/following")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.username} is Following · Verdict` },
      { name: "description", content: `People ${params.username} follows.` },
    ],
  }),
...
function FollowingPage() {
  ...
  useEffect(() => {
    getFollowingListFn({ data: { username } })
      .then(async (res) => {
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-2xl px-6 py-16">
        ...
        {loading ? (
          ...
          <p className="text-caption text-dust mt-10">Not following anyone yet.</p>
        ) : (
